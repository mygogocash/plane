...
**Status** ✅ done & verified (`0127_custom_properties.py`; 6 targeted tests + 246 backend unit tests pass)
...
- RED: `docker exec plane-tests pytest plane/tests/unit/db/test_issue_property_model.py -m unit -v --tb=short` failed at collection because `IssueProperty` was not exported yet.
- GREEN: `docker exec -w /code plane-tests pytest plane/tests/unit/db/test_issue_property_model.py -m unit -v --tb=short --create-db` → 6 passed.
- Regression: `docker exec -w /code plane-tests pytest plane/tests/unit -m unit -v --tb=short` → 246 passed, 4 pre-existing factory deprecation warnings.
- Migration: `docker exec -w /code plane-tests python manage.py makemigrations --check --dry-run` → no changes detected.
- Migration round-trip: `docker exec -w /code plane-tests sh -lc 'python manage.py migrate db 0126 --noinput && python manage.py migrate db 0127 --noinput && python manage.py migrate db 0126 --noinput && python manage.py migrate db 0127 --noinput'` → OK.
- Static checks: `docker exec -w /code plane-tests ruff check plane/db/models/issue_property.py plane/db/models/__init__.py plane/tests/factories.py plane/tests/unit/db/test_issue_property_model.py --fix`; `docker exec -w /code plane-tests ruff format ...`; `docker exec -w /code plane-tests python manage.py check` → clean.
...
- **Batch 0 (roots, fully parallel):** `CP-1-BE`, `DUP-1-API`. _(CP-1-BE unblocks the most; DUP-1-API is wholly independent.)_

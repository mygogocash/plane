...
Cloudflare Worker secrets / runtime variables:
...
- GitHub secret `MANUT_DIAGNOSTIC_TOKEN` when production diagnostic routes are
  smoke-tested

Deploy evidence resolves the active Worker version from Cloudflare's
`/workers/scripts/{script_name}/deployments` API after `wrangler deploy`.
Readiness rejects missing version IDs and raw git SHAs.
...
BLOCKED`. That is correct until final D1 import validation, authenticated
smoke, Better Stack checks, and explicit operator approval are recorded.
...
- D1 import evidence must include `summary`, `source_counts`,
  ...
  orphans. Required D1 table counts must be non-empty; all-zero required
  production counts are rejected.
...
- Authenticated smoke evidence must include every required workflow check with
  passing evidence, per-check `observed_at` timestamps, and explicit
  `cloudflare_route_verified` provenance.
- Better Stack cutover evidence must include the required `public-site`,
  ...
  `up`, every monitor must target the canonical production URL, and every
  required live endpoint probe must return HTTP `200` with its expected keyword.
...
Production diagnostic routes require `MANUT_DIAGNOSTIC_TOKEN`; the validation
tool sends it as a header for HTTP checks and as a query parameter for
WebSocket checks.
...
does not infer production from defaults for cutover evidence. It must also
prove Cloudflare route provenance with `cloudflare_route_verified: true` and a
non-empty `cloudflare_route_evidence` object.

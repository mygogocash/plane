...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "About Manut",
    description: "Work management for teams that need clear project state and careful AI-assisted workflows.",
  ...
  return (
    <>
      ...
      <main id="main" className="flex min-w-0 flex-col overflow-x-clip">
          ...
          <div className="container-prose">
            <Reveal className="mx-auto max-w-3xl text-center">
              ...
              <h1 className="mt-4 text-[clamp(2rem,4.2vw,3.5rem)] leading-[1.05] font-semibold tracking-[-0.025em] text-balance">
                ...
                <span className="display-italic text-gen-z-gradient">stays close to the work.</span>
              </h1>
              <p className="text-base text-muted-foreground sm:text-lg mt-6 text-pretty">
                Manut is operated by GoGoCash as a focused production app for team work state. The landing page now
                describes what the app can be checked against: the app entry point, source repository, support contact,
                monitored domains, and current access model.
              </p>
            </Reveal>
          </div>
        </section>

        <section aria-labelledby="values-heading" className="section-pad relative">
          <div className="container-prose">
            <Reveal className="mx-auto max-w-2xl text-center">
              <p className="kicker kicker-line">What we believe</p>
              <h2
                id="values-heading"
                className="mt-4 text-[clamp(1.75rem,3vw,2.5rem)] leading-[1.1] font-semibold tracking-[-0.025em] text-balance"
              >
            ...
            <Reveal
              delay={120}
              className="border-border bg-border [&>article]:bg-card mt-14 grid grid-cols-1 overflow-hidden rounded-2xl border md:grid-cols-3"
            >
              {values.map((v) => (
                <article
                  key={v.title}
                ...
                >
                  ...
                  <div>
                    <h3 className="text-lg text-foreground font-semibold tracking-tight">{v.title}</h3>
                    <p className="text-muted-foreground mt-2 max-w-prose text-[15px] leading-relaxed">{v.body}</p>
                  </div>
                </article>
              ))}
            </Reveal>
        ...
        <section className="section-pad relative">
          <div className="container-prose">
            <Reveal className="border-border bg-card mx-auto max-w-3xl rounded-2xl border p-8 sm:p-12">
              <p className="kicker kicker-line">The operator</p>
              <h2 className="mt-4 text-[clamp(1.5rem,2.6vw,2rem)] leading-[1.15] font-semibold tracking-[-0.025em] text-balance">
                Manut is built and operated by GoGoCash.
              </h2>
              <p className="text-muted-foreground sm:text-base mt-5 text-[15px] leading-relaxed text-pretty">
                GoGoCash maintains the public Manut identity, the app entry point, and the support path. The current
                landing page favors factual claims over broad category language so search engines, users, and AI answer
                engines describe Manut accurately.
              </p>

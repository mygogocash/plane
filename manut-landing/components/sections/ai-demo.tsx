...
import { BookOpen, FileText, ListChecks, Pencil, Search, Sparkles, Tags, Zap } from "lucide-react";
...
const MODE_TOOLS: Record<Mode, ReadonlyArray<string>> = {
  ...
  triage: ["Search work", "Read activity", "Draft summary", "Suggest owners", "Suggest labels"],
};
...
export function AiDemo() {
  ...
  return (
    <section id="ai" aria-labelledby="ai-heading" className="section-pad border-border relative border-y">
      <div aria-hidden className="bg-spectrum pointer-events-none absolute inset-x-0 top-0 h-96 opacity-70" />

      <div className="container-prose grid items-start gap-12 md:grid-cols-2 md:gap-16">
        <Reveal>
          <div className="border-border bg-card overflow-hidden rounded-2xl border shadow-[0_20px_60px_-20px_oklch(0_0_0/0.18)] dark:shadow-[0_20px_60px_-20px_oklch(0_0_0/0.55)]">
            <div className="border-border bg-muted/40 flex flex-col gap-3 border-b px-4 py-3.5 sm:flex-row sm:items-center sm:gap-3 sm:px-5">
              <div className="flex min-w-0 items-center gap-3">
                <span
                  aria-hidden
                  className="bg-foreground text-background grid size-7 shrink-0 place-items-center rounded-full"
                >
                  <Sparkles className="size-3.5" aria-hidden />
                </span>
                <span className="text-[13px] font-semibold tracking-tight">Manut AI assist</span>
              </div>
              ...
              >
                {(Object.keys(MODE_LABELS) as Mode[]).map((m) => (
                  <button
                    ...
                    onClick={() => setMode(m)}
                    className={cn(
                      "font-mono tracking-wider shrink-0 rounded-full px-2.5 py-1.5 text-[10px] uppercase transition-colors sm:py-1",
                      mode === m
            ...
            <div className="flex flex-col gap-4 p-4 sm:p-5">
              <div className="bg-foreground text-background ml-auto max-w-[85%] rounded-2xl rounded-br-md px-3.5 py-2.5 text-[13px]">
                Summarize the active cycle and prepare the next project handoff.
              </div>

              <div className="border-border bg-muted/40 max-w-[92%] rounded-2xl rounded-bl-md border px-4 py-3 text-[13px] leading-relaxed">
                <div className="font-mono tracking-widest text-muted-foreground mb-1.5 flex items-center gap-1.5 text-[10px] uppercase">
                  <Sparkles className="size-3" aria-hidden /> Manut AI · {MODE_LABELS[mode]}
                </div>
                {typed ? (
                  <>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {(mode === "triage"
                        ...
                        <Badge key={label} variant="secondary" className="font-mono rounded-full text-[10px]">
                          {label}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div aria-label="AI is typing" className="flex gap-1.5 py-1">
                    <span
                      aria-hidden
                      className="bg-muted-foreground size-1.5 animate-[bounce_1.2s_infinite] rounded-full"
                    />
                    <span
                      aria-hidden
                      className="bg-muted-foreground size-1.5 animate-[bounce_1.2s_infinite_0.2s] rounded-full"
                    />
                    <span
                      aria-hidden
                      className="bg-muted-foreground size-1.5 animate-[bounce_1.2s_infinite_0.4s] rounded-full"
                    />
                  </div>
                )}
              </div>
            ...
            <div className="border-border bg-background text-muted-foreground mx-5 mb-3 flex items-center gap-2 rounded-xl border px-3.5 py-2.5 text-[13px]">
                ...
                {ALL_TOOLS.map((t) => {
                  ...
                  return (
                    <span
                      key={t.id}
                      className={cn(
                        "font-mono tracking-wider inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] uppercase transition-colors",
                        enabled
        ...
        <Reveal delay={120}>
          ...
          <p className="text-base text-muted-foreground sm:text-lg mt-5 max-w-prose text-pretty">
            Manut can support summaries, drafts, triage, and next-step suggestions when AI is configured. The landing
            page keeps those claims factual and review-oriented.
          </p>
            ...
            {FEATURES.map((f) => (
              <li key={f.title} className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 text-[14px] leading-relaxed">{f.body}</p>
                </div>
              </li>
            ))}
          </ul>

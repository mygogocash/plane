...
function Feature({ title, description, icon, span = "default", accent, badges }: FeatureProps) {
  return (
    ...
    >
      ...
      <div>
        <h3 className="text-lg text-foreground font-semibold tracking-tight">{title}</h3>
        <p className="text-muted-foreground mt-2 max-w-prose text-[15px] leading-relaxed">{description}</p>
      </div>
...
export function Features() {
  return (
    <section id="features" aria-labelledby="features-heading" className="section-pad relative">
      <div className="container-prose">
        <Reveal className="mx-auto max-w-2xl text-center">
          <p className="kicker kicker-line">Work management</p>
          <h2
            id="features-heading"
            className="mt-4 text-[clamp(1.875rem,3.4vw,3rem)] leading-[1.1] font-semibold tracking-[-0.025em] text-balance"
          >
            The operational layer
            <br />
            <span className="display-italic text-gen-z-gradient">between planning and shipping.</span>
          </h2>
          <p className="text-base text-muted-foreground sm:text-lg mt-5 text-pretty">
            Manut keeps the everyday work visible: project scopes, work item detail, cycles, modules, intake, pages,
            attachments, and the signals a team needs to keep moving.
          </p>
        </Reveal>

        <Reveal
          delay={120}
          className="border-border bg-border [&>article]:bg-card mt-16 grid grid-cols-1 overflow-hidden rounded-2xl border md:grid-cols-3"
        >
          <Feature
            span="wide"
            ...
            badges={
              <div className="flex flex-wrap gap-1.5">
                <Badge variant="secondary" className="font-mono rounded-full text-[10px]">
                  Assignees
                </Badge>
                <Badge variant="secondary" className="font-mono rounded-full text-[10px]">
                  Labels
                ...
                <Badge className="bg-accent text-accent-foreground font-mono rounded-full text-[10px]">Activity</Badge>
              </div>
            }
          />
          <Feature

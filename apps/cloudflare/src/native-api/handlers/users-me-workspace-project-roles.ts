...
import { getWorkspaceBySlug, getWorkspaceProjectRoles, mapWorkspaceProjectRolesPayload } from "../db";
...
export async function handleUsersMeWorkspaceProjectRolesRequest(
  request: Request,
  ...
  const roles = await getWorkspaceProjectRoles(env, slug, user.id);
  if (isResponse(roles)) {
    return roles;
  }

  return jsonResponse(mapWorkspaceProjectRolesPayload(roles));
}
